...
#### Quickstart: Step 5 (Offline Dataset)

Build the offline dataset from the training split (uses 10 behavior policies):
...
- `data/processed/offline_dataset_train.npz` — ~16,000 transitions (15,960 = 10 episodes × 1,596 rows)

The dataset is generated by six core behavior policies (BuyAndHold, Random, TrendFollowing, MeanReversion, MACDCrossover, VolatilityRegime), three epsilon-greedy variants, and one mixture policy. See `src/data/behavior_policies.py` for the financial rationale behind each policy.

**Loading the dataset in downstream code:**

```python
from src.data.build_offline_dataset import load_offline_dataset, to_d3rlpy_dataset

data = load_offline_dataset("data/processed/offline_dataset_train.npz")
# data["observations"], data["actions"], data["rewards"], ...

# For d3rlpy training (remaps actions {-1,0,+1} → {0,1,2}):
mdp_dataset = to_d3rlpy_dataset(data)
```
